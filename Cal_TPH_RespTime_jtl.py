...
def load_jtl_data(jtl_files):
    ...
    try:
        if not jtl_files:
            raise ValueError("No JTL files provided.")
        dfs = []
        for idx, jtl_file in enumerate(jtl_files):
            try:
                ...
                print(f"❌ Error reading JTL file {idx + 1}: {e}")
                raise ValueError(f"Failed to read JTL file: {str(e)}")

        if not dfs:
            raise ValueError("No valid JTL data found in uploaded files.")

        return _normalize_jtl_data(pd.concat(dfs, ignore_index=True))
    except ValueError:
        raise
    except Exception as e:
        print(f"❌ Unexpected error loading JTL data: {e}")
        raise ValueError(f"Failed to load JTL data: {str(e)}")
...
def calculate_response_jtl(
    ...
    try:
        ...
        print(f"❌ Error in calculate_response_jtl: {e}")
        raise
...
def calculate_average_throughput(jtl_files, from_date=None, to_date=None, template=None, data=None):
    ...
    try:
        ...
        print(f"❌ Error in calculate_average_throughput: {e}")
        raise
